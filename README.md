...
```bash
git init
git add domhunt.py README.md pyproject.toml .gitignore
git commit -m "Initial commit: local .com RDAP domain hunter"
git branch -M main
git remote add origin git@github.com:YOURUSER/domhunt.git
git push -u origin main
```

## Safe first run

Start small before doing long exhaustive scans:

```bash
python3 domhunt.py --exact-length 4 --style brandable --resume --workers 2 --rate 1.5 --burst 2 --max-results 25 --max-checks 300
```

Then expand once you are happy with the behavior.
